...
def row_col_normalize(A):
    # To efficiently invert large D:
    D_out = torch.sum(A, dim=1)
    D_in = torch.sum(A, dim=0)

    D_in = torch.sqrt(D_in)
    D_in = 1 / D_in
    D_in = torch.diag(D_in)

    D_out = torch.sqrt(D_out)
    D_out = 1 / D_out
    D_out = torch.diag(D_out)

    # Efficient matrix multiplication of sparse matrices
    A_tilde = D_out.to_sparse_coo() @ A
    A_tilde = A_tilde.to_sparse_coo() @ D_in
...
# Source for the following function: https://github.com/gidiko/DiGAE/blob/main/Citation.py
...
def prepare_data(dataset, normalize, undirected=False, val_size=0.05, test_size=0.1, verbose=False):

    graph = load_npz_dataset(dataset)
    A = torch.tensor(graph['A'].toarray(), dtype=torch.float)
    X = torch.tensor(graph['X'].todense(), dtype=torch.float)
    X = F.normalize(X, p=2, dim=1)

    assert not (A == A.T).all()

    if verbose:
        print(f'NumNodes: {X.shape[0]}')
        print(f'NumEdges: {torch.sum(A)}')
        print(f'NumFeats: {X.shape[1]}')

    if undirected:
        A = torch.max(A, A.T)
        if verbose:
            print(f'NumEdges (undirected): {torch.sum(A)}')
    ...
    # Calculate A_tilde = D^-0.5 * A * D^-0.5 or D^-1 * A
    if undirected:
        ...
        A_tilde_train = row_normalize(A_train)
    elif normalize == 'row_col':
        A_tilde = row_col_normalize(A)
        A_tilde_train = row_col_normalize(A_train)
...
class VGAE(nn.Module):
    def __init__(self, in_features, hidden_dim, latent_dim):
        ...
        self.logistic_weights = nn.Parameter(torch.randn(latent_dim * 2 + 1))
    ...
    def forward(self, X, A_tilde):
        ...
        # Decoder
        undirected_A_hat = torch.sigmoid(Z @ Z.T)

        Z_i = Z.unsqueeze(1).expand(-1, Z.shape[0], -1)
        Z_j = Z.unsqueeze(0).expand(Z.shape[0], -1, -1)
        ones = torch.ones(Z.shape[0], Z.shape[0], 1)
        Z_cat = torch.cat([ones, Z_i, Z_j], dim=-1) # (nodes, nodes, latent_dim * 2 + 1)

        direction_logits = torch.sigmoid(Z_cat @ self.logistic_weights)

        A_hat = undirected_A_hat * direction_logits
...
class GAE(nn.Module):
    ...
    def loss(self, A_hat, A, _, __, reweigthing_A_pos, reweighting_reconstruction_error):
        ...
        weight = A * (reweigthing_A_pos - 1) + 1
        new_total_edges = weight.sum()
        weight = weight / new_total_edges * total_edges
...
def train(reweighting_reconstruction_error, reweighting_A_pos, variational, node_features, dataset, verbose, lr=0.01, hidden_dim=32, latent_dim=16, epochs=200):
    ...
    val_edges = data[dataset]['val']['edges']
    val_non_edges = data[dataset]['val']['non-edges']
...
undirected = False
data = {'cora': prepare_data('cora_ml.npz', normalize='row_col', undirected=undirected), 'citeseer': prepare_data('citeseer.npz', normalize='row_col', undirected=undirected)}
